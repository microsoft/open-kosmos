...
import { NativeMessageType, DEFAULT_SERVER_PORT } from 'chrome-mcp-shared';
...
export class NativeMessagingHost {
  ...
  private async handleMessage(message: any): Promise<void> {
    // Log all incoming messages from Extension
    console.error('[NativeMessagingHost] Received message from Extension:', JSON.stringify(message));
    
    if (!message || typeof message !== 'object') {
      console.error('[NativeMessagingHost] Invalid message format, rejecting');
    ...
    try {
      console.error(`[NativeMessagingHost] Processing message type: ${message.type}`);
      switch (message.type) {
        case NativeMessageType.START:
          console.error(`[NativeMessagingHost] Handling START - port: ${message.payload?.port}, browser: ${message.payload?.browser}`);
          await this.startServer(message.payload?.port || DEFAULT_SERVER_PORT, message.payload?.browser);
          break;
   ...
   * @param clientBrowser - The browser type from the connecting extension
   */
  private async startServer(port: number, clientBrowser?: string): Promise<void> {
    // Validate browser type before starting server
    const expectedBrowser = browserConfig.getBrowser();
    if (clientBrowser && clientBrowser !== expectedBrowser) {
      console.error(
        `[NativeMessagingHost] Browser mismatch: expected ${expectedBrowser}, got ${clientBrowser}`,
      );
      this.sendMessage({
        type: 'browser_mismatch',
        payload: {
          expected: expectedBrowser,
          received: clientBrowser,
          message: `Expected ${expectedBrowser}, but got ${clientBrowser}`,
        },
      });
