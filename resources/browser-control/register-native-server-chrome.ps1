# Native Server Registration Script for Chrome Native Messaging
# This script registers the Native Messaging host so Chrome Extension can communicate with it
...
# Fallback to default openkosmos-app location if not set
$UserDataDir = if ($env:OpenKosmos_USER_DATA_DIR) { 
    $env:OpenKosmos_USER_DATA_DIR 
} else { 
    Join-Path $env:APPDATA "openkosmos-app" 
}
...
# Chrome requires a subkey with the host name, and the (Default) value pointing to the manifest
