...
def check_openkosmos_requirements() -> bool:
...
class OpenKosmosAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info("[openkosmos] Connected and authenticated to %s", self._safe_url())
                return True
            return False
        except Exception as e:
            logger.error("[openkosmos] Connection failed: %s", e)
    ...
    async def _connect_and_auth(self) -> bool:
        ...
        try:
            ...
            logger.error("[openkosmos] WebSocket connect failed: %s", e)
            return False
        ...
        try:
            ...
            logger.error("[openkosmos] Failed to send auth message: %s", e)
        ...
        try:
            ...
            logger.error("[openkosmos] Failed to receive auth response: %s", e)
            await self._close_ws()
            return False
        ...
        if msg_type == "auth_success":
            ...
            logger.error("[openkosmos] Authentication failed: %s", error)
            self._set_fatal_error("auth_error", f"Auth failed: {error}", retryable=False)
            await self._close_ws()
            return False
        else:
            logger.error("[openkosmos] Unexpected auth response type: %s", msg_type)
            await self._close_ws()
            return False
    ...
    async def disconnect(self) -> None:
        ...
        logger.info("[openkosmos] Disconnected")
    ...
    async def _listen_loop(self) -> None:
        ...
        while self._should_reconnect:
            try:
                ...
                logger.debug("[openkosmos] Listener cancelled")
                return
            except ConnectionClosed as e:
                logger.warning("[openkosmos] Connection closed: code=%s reason=%s", e.code, e.reason)
                self._authenticated = False

                # Check if we should reconnect
                if e.code in AUTH_FAILURE_CODES:
                    logger.error("[openkosmos] Auth failure code %d - not reconnecting", e.code)
                ...
                logger.exception("[openkosmos] Listener error: %s", e)
                if self._should_reconnect:
                    await self._schedule_reconnect()
    ...
    async def _receive_messages(self) -> None:
        ...
        async for message in self._ws:
            try:
                ...
                logger.warning("[openkosmos] Received invalid JSON: %s", message[:100])
            except Exception as e:
                logger.exception("[openkosmos] Error handling message: %s", e)
    ...
    async def _handle_message(self, data: dict) -> None:
        ...
        if msg_type == "message":
            ...
            source = self.build_source(
                ...
                user_id="openkosmos_user",  # Single user per OpenKosmos instance
                user_name="User",
            )
            ...
            logger.debug(
                "[openkosmos] Received message: conversation=%s len=%d",
                conversation_id[:8],
                len(text),
            )
            ...
            logger.error("[openkosmos] Auth error received: %s", error)
            self._set_fatal_error("auth_error", f"Auth error: {error}", retryable=False)
            await self._notify_fatal_error()

        else:
            logger.debug("[openkosmos] Ignoring message type: %s", msg_type)
    ...
    async def _schedule_reconnect(self) -> None:
        ...
        logger.info(
            "[openkosmos] Reconnecting in %.1fs (attempt %d)",
            delay,
        ...
        try:
            ...
            if success:
                self._reconnect_attempt = 0
                self._mark_connected()
                logger.info("[openkosmos] Reconnected successfully")
        except Exception as e:
            logger.error("[openkosmos] Reconnect failed: %s", e)
    ...
    async def send(
        ...
        try:
            ...
            logger.debug(
                "[openkosmos] Sent reply: conversation=%s len=%d",
                conversation_id[:8],
                len(content),
            )
            ...
            logger.warning("[openkosmos] Send failed - connection closed: %s", e)
            return SendResult(
                success=False,
            ...
            logger.error("[openkosmos] Send failed: %s", e)
