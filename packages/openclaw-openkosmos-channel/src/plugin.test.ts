...
// Import after mocks
const { openkosmosPlugin } = await import('./plugin.js');

// Helper: create a minimal gateway context
function createGatewayCtx(overrides: Record<string, any> = {}) {
  return {
    cfg: {
      plugins: { entries: { openkosmos: { config: { url: 'ws://localhost:9527', accounts: { default: { token: 'test-token' } } } } } },
    },
    accountId: 'default',
...
describe('config resolution', () => {
  it('resolves account with valid config', () => {
    const cfg = {
      plugins: { entries: { openkosmos: { config: { accounts: { mybot: { token: 'abc123' } } } } } },
    };
    const result = openkosmosPlugin.config.resolveAccount(cfg as any, 'mybot');
    expect(result).toEqual({ accountId: 'mybot', token: 'abc123', configured: true });
  });

  it('falls back to default accountId when omitted', () => {
    const cfg = {
      plugins: { entries: { openkosmos: { config: { accounts: { default: { token: 'def' } } } } } },
    };
    const result = openkosmosPlugin.config.resolveAccount(cfg as any, null);
  ...
  it('returns configured: false when token is missing', () => {
    const cfg = {
      plugins: { entries: { openkosmos: { config: { accounts: { default: {} } } } } },
    };
    const result = openkosmosPlugin.config.resolveAccount(cfg as any, 'default');
    expect(result.configured).toBe(false);
    expect(result.token).toBe('');
  });

  it('returns empty account list when config is missing', () => {
    const ids = openkosmosPlugin.config.listAccountIds({} as any);
    expect(ids).toEqual([]);
  });

  it('isConfigured returns true when configured', () => {
    expect(openkosmosPlugin.config.isConfigured({ accountId: 'a', token: 't', configured: true })).toBe(true);
  });

  it('isConfigured returns false when not configured', () => {
    expect(openkosmosPlugin.config.isConfigured({ accountId: 'a', token: '', configured: false })).toBe(false);
  });
});
...
describe('gateway.startAccount', () => {
  ...
  it('sends auth message on ws open', async () => {
    ...
    await openkosmosPlugin.gateway!.startAccount!(ctx as any);
  ...
  it('returns early when url is not configured', async () => {
    const ctx = createGatewayCtx({
      cfg: { plugins: { entries: { openkosmos: { config: {} } } } },
    });
    await openkosmosPlugin.gateway!.startAccount!(ctx as any);
...
describe('gateway.stopAccount', () => {
  it('closes ws connection', async () => {
    ...
    await openkosmosPlugin.gateway!.stopAccount!({
      accountId: 'default',
      log: { info: vi.fn(), warn: vi.fn(), error: vi.fn() },
    } as any);
...
describe('outbound.sendText', () => {
  it('sends push with correct conversationId', async () => {
    ...
    const result = await openkosmosPlugin.outbound!.sendText!({
      text: 'hello',
      to: 'conv-123',
      accountId: 'default',
    } as any);
    ...
    expect(result.messageId).toMatch(/^openkosmos-/);
  });

  it('throws when no connected client', async () => {
    await expect(
      openkosmosPlugin.outbound!.sendText!({
        text: 'hello',
        to: 'conv-123',
