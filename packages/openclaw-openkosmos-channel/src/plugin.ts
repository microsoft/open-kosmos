...
function getPluginConfig(cfg: OpenClawConfig): Record<string, any> {
  return (cfg as any).plugins?.entries?.openkosmos?.config ?? {};
}
...
export const openkosmosPlugin: ChannelPlugin<ResolvedOpenKosmosAccount> = {
  id: CHANNEL_ID,
  meta: { id: 'openkosmos' as any, label: 'OpenKosmos', selectionLabel: 'OpenKosmos Desktop', docsPath: '/plugins/openkosmos', blurb: 'Connect OpenKosmos desktop app to OpenClaw' },
  ...
  setup: {
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const openkosmos = entries.openkosmos ?? {};
      const config = openkosmos.config ?? {};
      const accounts = config.accounts ?? {};
      accounts[accountId] = { ...accounts[accountId], token: input.token };
      return { ...cfg, plugins: { ...plugins, entries: { ...entries, openkosmos: { ...openkosmos, config: { ...config, accounts } } } } } as OpenClawConfig;
    },
  },
  ...
  outbound: {
    ...
    sendText: async ({ text, to, threadId, accountId }) => {
      ...
      return { channel: CHANNEL_ID as any, messageId: `openkosmos-${Date.now()}` };
    },
  },
};
