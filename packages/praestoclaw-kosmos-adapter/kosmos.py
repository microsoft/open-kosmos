...
class OpenKosmosChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not self._url:
            logger.error("[kosmos] No URL configured. Set OpenKosmos_URL env var")
            return
        if not self._token:
            logger.error("[kosmos] No token configured. Set OpenKosmos_TOKEN env var")
            return
    ...
    async def _process_message(self, data: dict[str, Any]) -> None:
        ...
        if msg_type == "message":
            ...
            await self._handle_message(
                ...
                sender_id="kosmos_user",  # Single user per OpenKosmos instance
                sender_name="User",
                content=text,
