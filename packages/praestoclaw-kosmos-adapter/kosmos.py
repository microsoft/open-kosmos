"""
Kosmos channel — WebSocket client connecting to Kosmos desktop app.
...
connecting to Kosmos's WS server.
...
class KosmosChannel(BaseChannel):
    """WebSocket client channel for Kosmos desktop app."""
    ...
    def __init__(
        ...
        if allow_from is None:
            allow_all = os.getenv("KOSMOS_ALLOW_ALL_USERS", "true").lower() in ("true", "1", "yes")
            if allow_all:
                allow_from = []  # empty = allow all

        super().__init__(bus, allow_from)

        self._url = url or os.getenv("KOSMOS_URL", "")
        self._token = token or os.getenv("KOSMOS_TOKEN", "")
    ...
    async def start(self) -> None:
        """Start the channel — begin connecting to Kosmos."""
        if not self._url:
            logger.error("[kosmos] No URL configured. Set KOSMOS_URL env var")
            return
        if not self._token:
            logger.error("[kosmos] No token configured. Set KOSMOS_TOKEN env var")
            return

        self._running = True
        self._connect_task = asyncio.create_task(self._connect_loop(), name="kosmos-connect")
        logger.info("[kosmos] Channel started, connecting to {}", self._safe_url())
    ...
    async def stop(self) -> None:
        ...
        logger.info("[kosmos] Channel stopped")

    async def send(self, channel_id: str, content: str, **kwargs: Any) -> None:
        """Send a response to Kosmos.

        channel_id is the conversationId from Kosmos.
        """
        if not self._ws or not self._authenticated:
            logger.warning("[kosmos] Cannot send — not connected")
            return
        ...
        try:
            ...
            logger.debug("[kosmos] Sent reply: conversation={} len={}", channel_id[:8], len(content))
        except websockets.exceptions.ConnectionClosed as e:
            logger.warning("[kosmos] Send failed — connection closed: {}", e)
        except Exception as e:
            logger.error("[kosmos] Send failed: {}", e)
    ...
    async def _connect_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.warning("[kosmos] Connection closed: code={} reason={}", e.code, e.reason)
                self._authenticated = False

                # Check for auth failure codes — don't reconnect
                if e.code in AUTH_FAILURE_CODES:
                    logger.error("[kosmos] Auth failure code {} — not reconnecting", e.code)
                    break

                if self._running:
                    await self._wait_and_reconnect()
            except Exception as e:
                logger.error("[kosmos] Connection error: {}", e)
                self._authenticated = False
                if self._running:
                    await self._wait_and_reconnect()
    ...
    async def _run_connection(self) -> None:
        ...
        logger.info("[kosmos] Connected and authenticated to {}", self._safe_url())
        self._reconnect_attempt = 0
    ...
    async def _connect_and_auth(self) -> bool:
        ...
        try:
            ...
            logger.error("[kosmos] WebSocket connect failed: {}", e)
            return False
        ...
        try:
            ...
            logger.error("[kosmos] Failed to send auth message: {}", e)
            await self._close_ws()
            return False
        ...
        try:
            ...
            logger.error("[kosmos] Failed to receive auth response: {}", e)
            await self._close_ws()
            return False
        ...
        if msg_type == "auth_success":
            ...
            logger.error("[kosmos] Authentication failed: {}", error)
            await self._close_ws()
            return False
        else:
            logger.error("[kosmos] Unexpected auth response type: {}", msg_type)
            await self._close_ws()
            return False
    ...
    async def _receive_messages(self) -> None:
        ...
        async for message in self._ws:
            try:
                ...
                logger.warning("[kosmos] Received invalid JSON: {}", message[:100] if message else "")
            except Exception as e:
                logger.exception("[kosmos] Error handling message: {}", e)
    ...
    async def _process_message(self, data: dict[str, Any]) -> None:
        ...
        if msg_type == "message":
            ...
            logger.debug(
                "[kosmos] Received message: conversation={} len={}",
                conversation_id[:8],
                len(text),
            )
            ...
            await self._handle_message(
                ...
                sender_id="kosmos_user",  # Single user per Kosmos instance
                sender_name="User",
                content=text,
                thread_id=conversation_id,
                metadata={"source": "kosmos"},
            )
            ...
            logger.error("[kosmos] Auth error received: {}", error)

        else:
            logger.debug("[kosmos] Ignoring message type: {}", msg_type)
    ...
    async def _wait_and_reconnect(self) -> None:
        ...
        logger.info("[kosmos] Reconnecting in {:.1f}s (attempt {})", delay, self._reconnect_attempt)
