...
The OpenKosmos application is based on the Electron framework and uses electron-builder for packaging. The following target platforms are currently supported:
- Windows x64 & ARM64
...
- Node.js >= 18.0.0
- npm or yarn
- Sufficient disk space (approximately 300–500 MB per platform)

### Platform-Specific Requirements

#### Windows Builds
- Windows 10/11 or macOS/Linux (via cross-compilation)
- Windows code signing certificate (if code signing is required)

#### macOS Builds
- macOS system (required — Apple restriction)
- Xcode Command Line Tools
- If code signing and notarization are required:
...
{
  "build": {
    "appId": "com.openkosmos-ai-studio",
    ...
    "mac": {
      "icon": "resources/icons/openkosmos_icon_1024x1024.png",
    ...
    "win": {
      "icon": "resources/icons/openkosmos_icon.ico",
      "target": [
        {
          "target": "nsis",
          "arch": ["x64", "arm64"]
...
Add new build commands to the scripts section of [`package.json`](package.json:9):
...
Create `scripts/build-all.js` as an automated build script:
...
echo 🎉 Windows platform builds complete!
...
echo "🎉 macOS platform builds complete!"
echo "📁 Output directory: release/"
```
...
If code signing is required, add the following to [`package.json`](package.json:98):
...
```bash
# Build for all platforms (requires the corresponding platform environment)
npm run dist:all
...
# Package only, without distributing (for testing)
npm run pack
```
...
After a successful build, installers will be generated in the `release/` directory:

```
release/
├── OpenKosmos-Setup-1.0.0-x64.exe           # Windows x64 installer
├── OpenKosmos-Setup-1.0.0-arm64.exe         # Windows ARM64 installer
├── OpenKosmos-Portable-1.0.0-x64.exe        # Windows x64 portable edition
├── OpenKosmos-Portable-1.0.0-arm64.exe      # Windows ARM64 portable edition
...
2. **Build failure: permission error**
   ```bash
   # Windows
