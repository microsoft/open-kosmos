...
OpenKosmos currently supports local MCP servers and unauthenticated online MCP servers, but it cannot successfully connect to OAuth-protected online MCP servers.
...
3. OpenKosmos surfaces a connection failure.
...
`The MCP Server Definition 'edge-growth-brain' wants to authenticate to Microsoft.`
...
OpenKosmos does not support the required product flow for authenticated online MCP servers:
...
As a result, any protected MCP resource behaves as permanently unavailable in OpenKosmos even though the same configuration works in VS Code.

## 3. Product Decision

OpenKosmos will add first-class support for OAuth-protected online MCP servers.

The product behavior will be:

1. when an online MCP server returns `401` or `403` with an OAuth challenge, OpenKosmos recognizes it as an authentication-required state rather than a terminal transport failure
2. OpenKosmos presents a consent prompt to the user before beginning authentication
3. after consent, OpenKosmos completes OAuth, obtains an access token, retries the original MCP request, and continues connection setup automatically
...
1. Make authenticated online MCP servers usable in OpenKosmos.
2. Align OpenKosmos user experience with VS Code for MCP OAuth scenarios.
...
1. "If an MCP server requires Microsoft login, OpenKosmos should prompt me instead of just failing."
...
2. Replacing the entire OpenKosmos MCP runtime with a different transport stack.
...
8. Supporting standards-compliant identity providers without provider-specific registration.

### 5.2 Out of Scope

1. Reworking stdio MCP server auth.
2. Redesigning unrelated product-specific browser authentication.
3. Historical auth session inspection UI.
4. Full identity-provider marketplace support in the first release.

## 6. User Stories

1. As a user, when I add a protected online MCP server, I want OpenKosmos to prompt me for sign-in rather than fail with a raw 401 error.
2. As a user, when I approve authentication, I want OpenKosmos to finish connecting automatically without requiring a manual reconnect.
3. As a user, when my authentication expires, I want OpenKosmos to ask me to re-authenticate in a clear and recoverable way.
4. As a developer/operator, I want OpenKosmos logs and state to clearly distinguish auth-required states from generic transport failures.

## 7. Experience Requirements

### 7.1 Consent Behavior

Before starting an OAuth flow, OpenKosmos must ask for user consent.
...
After successful authentication, OpenKosmos must automatically retry the MCP request that failed because of missing auth.
...
OpenKosmos should distinguish among:
...
1. A protected online MCP server such as `edge-growth-brain` can be added and connected successfully in OpenKosmos.
...
1. With the `edge-growth-brain` config, OpenKosmos no longer fails immediately on the initial `401`.
2. OpenKosmos shows an authentication consent prompt.
3. After the user approves and completes auth, OpenKosmos retries automatically and connects.
...
1. Adding provider-specific assumptions could make standards-compliant MCP OAuth implementations fail.
...
OpenKosmos will support OAuth-protected online MCP servers by turning authentication challenges into a guided consent-and-login flow followed by automatic authenticated retry, beginning with Microsoft identity-compatible servers such as `edge-growth-brain`.