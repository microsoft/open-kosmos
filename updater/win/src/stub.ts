...
const LOG_FILE = path.join(os.tmpdir(), 'kosmos-updater.log');
...
# Log file path
$script:logPath = Join-Path $env:TEMP "kosmos-updater.log"
...
function Start-UpdateProcess {
    ...
    $tempDir = Join-Path $tempBase "kosmos-update-$(Get-Date -Format 'yyyyMMddHHmmss')"
...
function getEmbeddedScriptPath(): string {
  const tempScriptPath = path.join(os.tmpdir(), 'kosmos-updater-ui.ps1');
...
function runPowerShellUpdater(zipPath: string, installPath: string, options: UpdateOptions = {}): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    const vbsPath = path.join(os.tmpdir(), 'kosmos-updater-launcher.vbs');
    const vbsContent = `
...
async function runSilentUpdate(zipPath: string, installPath: string): Promise<boolean> {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `kosmos-update-${Date.now()}`);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const vbsPath = path.join(os.tmpdir(), `kosmos-updater-${Date.now()}.vbs`);
