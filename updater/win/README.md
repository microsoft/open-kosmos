...
Log file location: `%TEMP%\kosmos-updater.log`

View logs:
```powershell
Get-Content $env:TEMP\kosmos-updater.log
```
...
| File | Purpose |
|------|---------|
| `%TEMP%\kosmos-updater-ui.ps1` | PowerShell UI script |
| `%TEMP%\kosmos-updater-*.vbs` | VBScript launcher |
| `%TEMP%\kosmos-update-*\` | Extracted update files |
...
1. Check `%TEMP%\kosmos-updater.log` for errors
