...
const testTexts = [
    ...
    "GitHub Copilot uses machine learning to assist developers in coding tasks."
];

async function testEmbedding3Small() {
    try {
        console.log('=== GitHub Copilot text-embedding-3-small model test ===\n');

        // Request body — using correct format based on source code analysis
        const requestBody = {
            input: testTexts,  // use input instead of inputs
            model: "text-embedding-3-small",
            dimensions: 1536  // default dimensions for 3-small
        };
        ...
        if (response.statusCode === 200) {
            ...
            if (responseData.data && Array.isArray(responseData.data)) {
                console.log(`\nSuccessfully retrieved embeddings for ${responseData.data.length} texts:`);
                ...
                if (responseData.data.length >= 2) {
                    ...
                    if (vec1 && vec2) {
                        ...
                        console.log(`Cosine similarity between text 1 and text 2: ${similarity.toFixed(6)}`);
                    }
                }
            }
        } else {
