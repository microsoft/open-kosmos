...
Main-process dynamic chunks must use stable names (`chunkFileNames: '[name].js'`), not content hashes. Lazy built-in tools can be imported long after Electron starts; if a rebuild changes chunk hashes while the app is still running, the in-memory `main.js` points at deleted filenames and tools fail with `Cannot find module './<tool>-<oldhash>.js'`.
...
All scripts support the `--brand=openkosmos` flag. `npm run dev` specifies the entry via `ELECTRON_ENTRY=dist-vite/main/main.js`.
...
- **Brand icon loading**: The previous `` require(`../../assets/${BRAND_NAME}/app.svg`) `` pattern was replaced with a new module `src/renderer/lib/brandIcon.ts` that statically imports the OpenKosmos SVG and exposes the selected app icon. Related components (WindowsTitleBar, StartupPage, AboutAppContentView) all reference this module.
