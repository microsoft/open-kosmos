...
- **Multiple AI Providers**: Support for OpenAI, Google Gemini, Claude Sonnect
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/gim-home/Kosmos.git
   cd Kosmos
   ```
...
To contribute to OpenKosmos, please contact [contact@example.com](mailto:contact@example.com) to request development access.
...
2. The `EVAL_AUTH_TOKEN` environment variable **must be set**. This token is generated by [AgenticEval](https://github.com/JuntongLiu96/agentic-eval) and passed to Kosmos via the adapter config. All requests (except `/eval/health`) must include `Authorization: Bearer <token>`. You can set it in either of two ways:
...
- Port 8100 must not conflict with `BrowserControlHttpServer` (port 8000).
...
// Dynamic value: function that receives context
{
  ...
  description: 'Show Toolbar entry in Settings page',
  defaultValue: (ctx) => ctx.brandName === 'kosmos',  // Enabled only for kosmos brand
  devOnly: true,
}
```
...
- **GitHub**: [gim-home/Kosmos](https://github.com/gim-home/Kosmos)
