...
describe('electronAPI.openkosmos', () => {
  it('replacePlaceholders', () => {
    api.openkosmos.replacePlaceholders({ KEY: 'value' });
    expect(mocks2.mockInvoke).toHaveBeenCalledWith('openkosmos:replacePlaceholders', { KEY: 'value' });
  });

  it('parseUserInputPlaceholders', () => {
    api.openkosmos.parseUserInputPlaceholders({ config: 'data' });
    expect(mocks2.mockInvoke).toHaveBeenCalledWith('openkosmos:parseUserInputPlaceholders', { config: 'data' });
  });
});
