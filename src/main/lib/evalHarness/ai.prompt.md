<!-- Last verified: 2026-07-11 (evalAgentRunner extracts sub-agent messages from retained `sub_agent` tool results.) -->
# Eval Harness
...
## Architecture
- Activated via `--eval-mode` flag in `main.ts` `onReady()`. Skips window creation and initializes only auth, profile, MCP, and chat singletons. Startup logic lives in `src/main/startup/evalMode.ts`.
...
- ⚠️ Port 8100 must not conflict with other local services.
