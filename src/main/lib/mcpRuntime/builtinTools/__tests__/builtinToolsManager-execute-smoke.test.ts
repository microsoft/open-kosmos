...
vi.mock('electron', async () => ({
  app: {
    getPath: vi.fn((name: string) => {
      if (name === 'userData') {
        return path.join(os.tmpdir(), 'kosmos-vitest-userdata');
      }
      return os.tmpdir();
    }),
  },
...
vi.mock('../../../featureFlags', async () => ({
  isFeatureEnabled: vi.fn((name: string) => {
    if (name === 'kosmosFeatureSubAgent') return mockSubAgentFeatureEnabled.value;
    if (name === 'kosmosFeatureRemoteChannel') return mockRemoteChannelEnabled.value;
    if (name === 'kosmosFeatureScheduler') return mockSchedulerEnabled.value;
...
describe('BuiltinToolsManager — execute smoke coverage', () => {
  ...
  beforeEach(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'kosmos-builtin-smoke-'));
