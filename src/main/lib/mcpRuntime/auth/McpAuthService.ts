...
function pickAuthUiWindow(windows: ElectronBrowserWindow[]): ElectronBrowserWindow | null {
  const usableWindows = windows.filter((window) => !window.isDestroyed() && !!window.webContents);
  return usableWindows.find((window) => {
    try {
      const url = window.webContents.getURL();
      return !window.getParentWindow()
        && !url.includes('screenshot.html')
        && !url.includes('/screenshot')
        && !/debug tools/i.test(window.getTitle());
    } catch {
      return false;
    }
  }) ?? usableWindows[0] ?? null;
}
...
export class McpAuthService {
  ...
  /** Acquire an access token through the provider-neutral MCP OAuth flow. */
