...
export const MCP_DCR_RESTRICTED_CODE = 'MCP_DCR_RESTRICTED';

export function createMcpDcrRestrictedError(serverName: string, cause: string): Error {
  return new Error(`[${MCP_DCR_RESTRICTED_CODE}] MCP server "${serverName}" restricts connections to approved clients only. This app is not currently supported by this server. Check the server's documentation for a list of supported clients. (Server response: ${cause})`);
}

export function isMcpDcrRestrictedError(error: Error | null | undefined): boolean {
  return !!error && error.message.startsWith(`[${MCP_DCR_RESTRICTED_CODE}]`);
}
