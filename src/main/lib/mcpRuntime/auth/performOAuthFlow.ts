 ...
 * `auth()` helper. Throws cancelled / DCR-restricted / DCR-fallback-required /
...
export async function performOAuthFlow(
  ...
  try {
    ...
    if (isDcrRestrictedError(e)) {
      logger.info(`[McpOAuth] DCR restricted by ${serverName}; server only allows approved clients`, 'performOAuthFlow', {
        underlying: errorMessage(e),
      });
      throw createMcpDcrRestrictedError(serverName, errorMessage(e));
    }
 ...
 */
/**
 * DCR is restricted to approved partners — the server explicitly rejects
 * unapproved clients. No user action can fix this.
 */
function isDcrRestrictedError(e: unknown): boolean {
  if (!(e instanceof Error)) return false;
  const m = e.message.toLowerCase();
  return m.includes('dynamic client registration') && m.includes('restricted');
}

/**
 * DCR is not supported at all — the user may be able to provide a
 * manually-registered client_id.
 */
