...
describe('McpAuthMetadataService.resolve', () => {
  ...
  it('normalizes generic terminal /authorize URLs', async () => {
    const authorizationUri = 'https://auth.example.com/tenant/authorize';
    vi.spyOn(global, 'fetch' as any)
      .mockResolvedValueOnce(new Response(null, { status: 404 })) // resource metadata
      .mockResolvedValueOnce(new Response(JSON.stringify({
    ...
    expect(result?.authorizationServerUrl).toBe('https://auth.example.com/tenant');
    expect(result?.providerLabel).toBe('Identity Provider');
  });
});
