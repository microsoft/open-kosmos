...
describe('getMcpOAuthServerKey', () => {
  ...
  it('does NOT change when only display fields change', () => {
    ...
    const b = makeCfg({ version: '2.0.0', remoteVersion: '2.1.0', source: 'ON-DEVICE' });
    expect(getMcpOAuthServerKey('s', a)).toBe(getMcpOAuthServerKey('s', b));
  });
  ...
  it('treats an omitted URL the same as an explicit empty URL', () => {
    const withoutUrl = makeCfg({ url: undefined, headers: undefined });
    const emptyUrl = makeCfg({ url: '', headers: undefined });
    expect(getMcpOAuthServerKey('s', withoutUrl)).toBe(getMcpOAuthServerKey('s', emptyUrl));
  });
