 ...
 * for provider-neutral MCP servers.
 *
 * Focus areas:
...
const openExternalMock = vi.hoisted(() => vi.fn(async () => true));
vi.mock('electron', () => ({
...
describe('OpenKosmosOAuthProvider', () => {
  describe('redirectUrl + clientMetadata', () => {
    ...
    it('pins a configured callback port and opens authorization in the system browser', async () => {
      const { OpenKosmosOAuthProvider } = await import('../OpenKosmosOAuthProvider');
      const p = new OpenKosmosOAuthProvider('s', makeCfg({ oauth: { callbackPort: 4567 } }));
      expect(p.pinnedCallbackPort).toBe(4567);
      await expect(p.redirectToAuthorization(new URL('https://auth.example.test/authorize?q=1')))
        .resolves.toBeUndefined();
      expect(openExternalMock).toHaveBeenCalledWith('https://auth.example.test/authorize?q=1');
    });

    it.each([new Error('browser unavailable'), 'browser unavailable'])(
      'keeps manual browser navigation possible when shell launch rejects with %s',
      async (failure) => {
        const { OpenKosmosOAuthProvider } = await import('../OpenKosmosOAuthProvider');
        openExternalMock.mockRejectedValueOnce(failure);
        await expect(new OpenKosmosOAuthProvider('s', makeCfg()).redirectToAuthorization(
  ...
  describe('markAccessTokenExpired', () => {
    ...
    it('is a no-op for an already empty and expired cache slot', async () => {
      const { OpenKosmosOAuthProvider } = await import('../OpenKosmosOAuthProvider');
      const p = new OpenKosmosOAuthProvider('s', makeCfg());
      storeImpl[p.debugServerKey] = {
        serverName: 's',
      ...
      await p.markAccessTokenExpired();
      expect(cacheMock.setMcpOAuth).not.toHaveBeenCalled();
    });
  });
  ...
  describe('invalidateCredentials', () => {
    ...
    it('is a no-op when no cache slot exists', async () => {
      const { OpenKosmosOAuthProvider } = await import('../OpenKosmosOAuthProvider');
      const p = new OpenKosmosOAuthProvider('s', makeCfg());
      await expect(p.invalidateCredentials('tokens')).resolves.toBeUndefined();
      expect(cacheMock.setMcpOAuth).not.toHaveBeenCalled();
    });
  });
