...
vi.mock('../../auth/McpAuthService', async () => ({
  ...
  setSkipNextMsalBrowserConsentOnce: vi.fn(),
}));
...
describe('VscodeHttpTransport auth retry', () => {
  ...
  it('retries with authorization header after resolving auth metadata', async () => {
    ...
    mockResolveMetadata.mockResolvedValue({
      authorizationServerUrl: 'https://auth.example.com/default/v2.0',
      authorizationServerMetadata: {
        issuer: 'https://auth.example.com/default/v2.0',
        authorization_endpoint: 'https://auth.example.com/default/oauth2/v2.0/authorize',
        token_endpoint: 'https://auth.example.com/default/oauth2/v2.0/token',
      },
  ...
  it('reuses auth metadata before the first fetch and force-refreshes after an authorized 401', async () => {
    ...
    expect(mockGetTokenForServer).toHaveBeenNthCalledWith(1, 'test-mcp',
      expect.objectContaining({ scopes: ['api://resource/.default'] }),
      expect.objectContaining({ cfg: undefined }));
    expect(mockGetTokenForServer).toHaveBeenNthCalledWith(2, 'test-mcp',
      expect.objectContaining({ scopes: ['api://resource/.default'] }),
      expect.objectContaining({ cfg: undefined }));
    expect(mockGetTokenForServer).toHaveBeenNthCalledWith(3, 'test-mcp',
      expect.objectContaining({ scopes: ['api://resource/.default'] }),
