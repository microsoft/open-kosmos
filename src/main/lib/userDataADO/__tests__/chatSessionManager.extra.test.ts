...
describe('ChatSessionManager.extra', () => {
  ...
  describe('writeFileAtomically (internal)', () => {
    ...
    it('retries a transient EPERM rename failure and then succeeds', async () => {
      vi.useFakeTimers();
      try {
        (fs.promises.writeFile as any).mockResolvedValue(undefined);
        (fs.promises.rename as any)
          .mockRejectedValueOnce(Object.assign(new Error('EPERM: operation not permitted, rename'), { code: 'EPERM' }))
          .mockResolvedValueOnce(undefined);

        const promise = (manager as any).writeFileAtomically('/some/path/index.json', '{}');
