...
import { formatLinkStats } from "../lib/formatter";
...
export async function organizeCommand(
  ...
  const stats = cards.map((card) => ({
    slug: card.slug,
    // VENDOR PATCH (P6): outboundMap/inboundMap are populated for every card.slug
    // in the loops above, so the `|| []` fallbacks are unreachable defensive code.
    /* v8 ignore next 2 */
  ...
  if (recentCards.length > 0) {
    ...
    for (const slug of recentCards) {
      ...
      // VENDOR PATCH (P6): every recentCards slug has a cardData entry, so this
      // guard and the outboundMap `|| []` fallback below are unreachable.
      /* v8 ignore next */
      if (!info) continue;
  ...
  if (json) {
    const jsonOutput = {
      stats,
      // VENDOR PATCH (P6): cardData.title is always a non-empty string (slug
      // fallback applied at build time), so the `?? slug` arms are unreachable.
      /* v8 ignore next 3 */
