...
describe('AgentChatStreamingService', () => {
  ...
  describe('callWithToolsStreaming - image stats logging', () => {
    it('logs image stats when messages contain image_url content blocks', async () => {
      mockedGetEndpointForModel.mockReturnValue('/chat/completions' as any);
      const reader = createReader([
        'data: {"choices":[{"delta":{"content":"described"},"finish_reason":"stop"}]}\n',
        'data: [DONE]\n',
      ]);
      ...
      const result = await service.callWithToolsStreaming();
      expect(result.message.content).toEqual([{ type: 'text', text: 'described' }]);

      const imageLog = consoleSpy.mock.calls.find(
        (args: any[]) => typeof args[0] === 'string' && args[0].includes('image stats'),
      );
      expect(imageLog).toBeDefined();
      expect(imageLog![1]).toMatchObject({
        imageCount: 2,
      });
