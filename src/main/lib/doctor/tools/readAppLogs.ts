...
- \`scope\`: \`"current"\` (default — only this run's logs: dev mode = this launch's \`kosmos-dev-*.log\`; prod mode = today's \`kosmos-YYYY-MM-DD.log\`) or \`"all"\` (across all historical log files, including prior dev launches)
    ...
    parameters: {
      ...
      properties: {
        ...
        scope: {
          type: 'string',
          enum: ['current', 'all'],
          description: `Which log files to query. Default "current" — the current run only (dev: this launch's kosmos-dev-*.log; prod: today's kosmos-YYYY-MM-DD.log). "all" = every log file in the directory (including prior dev launches).`,
        },
      },
      required: ['mode'],
    },
...
function resolveLogFiles(logsDir: string, scope: 'current' | 'all'): string[] {
  ...
  // Dev mode = this launch's kosmos-dev-*.log; prod = today's kosmos-YYYY-MM-DD.log.
