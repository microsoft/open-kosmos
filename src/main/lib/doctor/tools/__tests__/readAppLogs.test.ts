...
const { mockGetCurrentLogFileName, mockGetDefaultLogDirectory, mockIsDevelopmentLogEnvironment } = vi.hoisted(() => ({
  mockGetCurrentLogFileName: vi.fn(() => 'openkosmos-dev-2026-05-17-10-00-00.log'),
...
function setupLogFiles(content: string, filename = 'openkosmos-dev-2026-05-17-10-00-00.log') {
...
describe('executeReadAppLogs — scope=all', () => {
  ...
  it('reads all log files', async () => {
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.readdirSync).mockReturnValue([
      'openkosmos-2026-05-16.log',
      'openkosmos-2026-05-17.log',
    ] as any);
...
describe('executeReadAppLogs — edge cases', () => {
  ...
  it('falls back to most recent file when current file missing', async () => {
    vi.mocked(fs.existsSync).mockImplementation((p: any) => {
      ...
      if (p === '/mock/logs/openkosmos-dev-2026-05-17-10-00-00.log') return false;
      return false;
    });
    vi.mocked(fs.readdirSync).mockReturnValue(['openkosmos-2026-05-16.log'] as any);
    vi.mocked(fs.statSync).mockReturnValue({ size: SAMPLE_LOG.length, mtimeMs: Date.now() } as any);
    vi.mocked(fs.readFileSync).mockReturnValue(SAMPLE_LOG);
    const r = await executeReadAppLogs({ mode: 'stats' });
  ...
  it('handles prod mode scope notice', async () => {
    ...
    mockGetCurrentLogFileName.mockReturnValue('openkosmos-2026-05-17.log');
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.readdirSync).mockReturnValue(['openkosmos-2026-05-17.log'] as any);
    vi.mocked(fs.statSync).mockReturnValue({ size: SAMPLE_LOG.length, mtimeMs: Date.now() } as any);
    vi.mocked(fs.readFileSync).mockReturnValue(SAMPLE_LOG);
    const r = await executeReadAppLogs({ mode: 'stats' });
  ...
  it('handles prod mode scope notice — fallback case', async () => {
    ...
    vi.mocked(fs.existsSync).mockImplementation((p: any) => {
      if (p === '/mock/logs') return true;
      if (p === '/mock/logs/openkosmos-2026-05-17.log') return false;
      return false;
    });
    vi.mocked(fs.readdirSync).mockReturnValue(['openkosmos-2026-05-16.log'] as any);
    vi.mocked(fs.statSync).mockReturnValue({ size: SAMPLE_LOG.length, mtimeMs: Date.now() } as any);
  ...
  it('handles read failure on a file', async () => {
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.readdirSync).mockReturnValue(['openkosmos-dev-2026-05-17-10-00-00.log'] as any);
