...
describe('formatStalenessHeader', () => {
  ...
  it('includes file names', () => {
    ...
    const result = formatStalenessHeader([entry], ['/logs/kosmos-2026-05-17.log']);
    expect(result).toContain('kosmos-2026-05-17.log');
  });

  it('includes last log entry timestamp', () => {
    const entry = makeEntry({ timestamp: new Date('2026-05-17T10:00:00Z') });
    const result = formatStalenessHeader([entry], ['/logs/kosmos.log']);
