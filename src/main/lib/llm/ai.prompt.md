...
| `textLlmEmbedder.ts` | `TextLlmEmbedder` — embedding adapter used by the memory system | small |
...
- **Streaming** in the main `AgentChat` uses Vercel AI SDK 5.x directly with provider-specific adapters (openai-compatible, google-generative-ai, cohere, ollama). `GhcModelApi` is a non-streaming single-call utility only.
...
- Multiple provider support (OpenAI, Gemini, Claude, Cohere, Ollama) is handled in `AgentChat` via Vercel AI SDK factory functions; the files here cover OpenKosmos-internal utility calls only.
