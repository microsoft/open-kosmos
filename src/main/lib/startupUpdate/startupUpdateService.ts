  ...
  private async installMcpUpdates(): Promise<number> {
    ...
    for (const { local, remote } of this.mcpUpdates) {
      try {
        ...
        try {
          processedEnv = kosmosPlaceholderManager.replacePlaceholdersInObject(mergedEnv, { alias: this.alias });
        } catch (e) {
        ...
        if (processedUrl) {
          try {
            if (containsOpenKosmosPlaceholder(processedUrl)) {
              processedUrl = kosmosPlaceholderManager.replacePlaceholders(processedUrl, { alias: this.alias });
            }
          } catch (e) {
            // Keep original URL
          }
