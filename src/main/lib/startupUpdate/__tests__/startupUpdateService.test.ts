...
describe('StartupUpdateService - Steps 0-6', () => {
  ...
  beforeEach(async () => {
    ...
    const { openkosmosPlaceholderManager, containsOpenKosmosPlaceholder } = await import('../../userDataADO/openkosmosPlaceholders');
    vi.mocked(openkosmosPlaceholderManager.replacePlaceholdersInObject).mockImplementation((obj: any) => obj);
    vi.mocked(openkosmosPlaceholderManager.replacePlaceholders).mockImplementation((str: string) => str);
  ...
  describe('checkMcpUpdates / installMcpUpdates (Steps 1-2)', () => {
    ...
    it('should process URL with openkosmos placeholder when containsOpenKosmosPlaceholder returns true', async () => {
      const { McpLibraryFetcher } = await import('../../assetsFetcher/mcpLibraryFetcher');
      vi.mocked(McpLibraryFetcher.getInstance).mockReturnValue({
        fetchAndUpdate: vi.fn().mockResolvedValue({
          success: true,
      ...
      vi.mocked(openkosmosPlaceholderManager.replacePlaceholders).mockReturnValueOnce('https://resolved-host/api');
    ...
    it('should be non-fatal when placeholder replacement throws', async () => {
      ...
      const { openkosmosPlaceholderManager } = await import('../../userDataADO/openkosmosPlaceholders');
      vi.mocked(openkosmosPlaceholderManager.replacePlaceholdersInObject).mockImplementationOnce(() => { throw new Error('placeholder error'); });
    ...
    it('should keep original URL when replacePlaceholders throws for URL placeholder', async () => {
      const { containsOpenKosmosPlaceholder } = await import('../../userDataADO/openkosmosPlaceholders');
      vi.mocked(containsOpenKosmosPlaceholder).mockReturnValueOnce(false).mockReturnValueOnce(true);
      const { openkosmosPlaceholderManager } = await import('../../userDataADO/openkosmosPlaceholders');
      vi.mocked(openkosmosPlaceholderManager.replacePlaceholders).mockImplementationOnce(() => { throw new Error('url placeholder error'); });
