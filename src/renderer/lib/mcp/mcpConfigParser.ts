...
export function parseMcpConfig(
  ...
  try {
    ...
    if (transportType === 'stdio') {
      ...
      if (
        extractedConfig.headers &&
        typeof extractedConfig.headers === 'object' &&
        !Array.isArray(extractedConfig.headers) &&
        Object.keys(extractedConfig.headers).length > 0
      ) {
        if (Object.values(extractedConfig.headers).every((v: unknown) => typeof v === 'string')) {
          config.headers = extractedConfig.headers
        } else {
          return {
            success: false,
...
export function formatToMcpServersWrapper(parsedConfig: ParsedMcpConfig): string {
  ...
  if (transportType !== 'stdio' && config.headers && Object.keys(config.headers).length > 0) {
...
export function formatToVSCodeSettings(serverConfigs: OpenKosmosAppMCPServerConfig[]): string {
  ...
  for (const config of serverConfigs) {
    ...
    if (config.transport === 'stdio') {
      ...
      // HTTP transports may include headers
      if (config.headers && Object.keys(config.headers).length > 0) {
...
export function parseVSCodeConfigToInternal(
  ...
  try {
    ...
    if (transportType === 'stdio') {
      ...
      if (
        firstServerConfig.headers &&
        typeof firstServerConfig.headers === 'object' &&
        !Array.isArray(firstServerConfig.headers) &&
        Object.keys(firstServerConfig.headers).length > 0
      ) {
        if (Object.values(firstServerConfig.headers).every((v: unknown) => typeof v === 'string')) {
          config.headers = firstServerConfig.headers
        } else {
          return {
            success: false,
            error: 'All header values must be strings'
