/**
 * OpenKosmos Streaming v2 configuration management and feature switches
 * Provides dynamic configuration, feature flags, and performance tuning
 */
...
export const DEFAULT_PERFORMANCE_CONFIG: StreamingV2PerformanceConfig = {
  targetFPS: 120, // High frame rate for smooth rendering
  adaptiveThrottling: false, // 🚀 Disable throttling for maximum speed
  memoryOptimization: true,
  bufferSize: 200, // 🚀 Larger buffer to support ultra-fast output
  flushThreshold: 300 // 🚀 Lower threshold for faster output
};
...
export const DEFAULT_UI_CONFIG: StreamingV2UIConfig = {
  ...
  renderingMode: 'immediate' // Immediate rendering mode for maximum output speed
};
...
export class StreamingConfigManager {
  ...
  constructor() {
    // Always use default config from code; do not load from localStorage
  ...
  // ========== Agent-specific Config Management ==========
  ...
  getAgentConfig(agentId: string): AgentStreamingConfig {
    ...
    // Return default agent config based on global config
    return {
      agentId,
  ...
  private setupPerformanceMonitoring(): void {
    // Monitor performance metrics and auto-adjust config
    if (typeof window !== 'undefined') {
      ...
      setInterval(() => {
        if (this.globalConfig.enableAdaptiveOptimization) {
          ...
          // Only optimize during the first few checks; reduce frequency after that
  ...
  private checkAndOptimizePerformance(): void {
    ...
    if (memoryInfo) {
      ...
      // Stricter threshold to reduce unnecessary optimizations
  ...
  private optimizeForMemory(): void {
    ...
    // Reduce update frequency to avoid frequent config change triggers
  ...
  // Silent update methods — do not notify listeners
  private updateGlobalConfigSilent(updates: Partial<StreamingV2Config>): void {
    this.globalConfig = { ...this.globalConfig, ...updates };
    // Do not call notifyConfigListeners() to avoid frequent updates
  }
