...
import { getBuddySpeciesLabel } from './buddyLabels';
...
export const BuddyCard: React.FC<Props> = ({ entry, isActive, isSelected, onActivate, onSelect, onShowStats }) => {
  ...
  return (
    ...
    >
      ...
      <div className="buddy-card-level">{t('buddy.levelCompact', { level })}</div>
      <div className="buddy-card-species" style={{ color }}>
        {getBuddySpeciesLabel(t, bones.species)}
      </div>
      {isActive && <div className="buddy-card-active-badge">{t('buddy.active')}</div>}
    </div>
  );
};
