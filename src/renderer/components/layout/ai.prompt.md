...
> Shell of the renderer SPA: left navigation sidebar, resizable divider, main content area, Windows title bar, and the `LayoutProvider` context that tracks minimal-mode/always-on-top state.

## Key Files
| File | Responsibility | Size |
|------|---------------|------|
| `AppLayout.tsx` | Root layout component — wraps `LayoutProvider` and `PasteToWorkspaceProvider`, owns global event listeners (chat-session delete/rename/star/download/fork, debug-info download), knowledge-base and skill-install callbacks, and renders `AppLayoutContent` | ~320 LOC |
| `AppLayoutContent.tsx` | UI shell — macOS titlebar/zoom/fullscreen handling, left sidebar + `ResizableDivider` + `ContentContainer`, global dropdown menus, overlays, and Buddy | ~270 LOC |
...
| `NavigationSection.tsx` | Agent list area: "New Agent" button, scrollable `AgentList`, `Divider`, and built-in agents below divider | ~432 LOC |
...
- **Component split**: The former monolithic `AppLayout` (~2500 LOC) has been decomposed into two layers:
  ...
  - `AppLayoutContent` — pure UI: sidebar and content layout, all global dropdown menus and overlays are rendered here as sibling portals. This is where macOS titlebar, zoom-factor sync, and fullscreen CSS class management live.
- **State ownership**: `LayoutProvider` wraps the entire app and is the single source of truth for minimal mode and always-on-top. Sidebar state is managed by two atoms: `LeftNavSizeAtom` (width, drag, persistence) and `LeftNavCollapsedAtom` (collapse toggle, persistence) in `src/renderer/states/left-nav.atom.ts`.
- **Minimal mode**: enabling it automatically sets `alwaysOnTop = true` via `electronAPI.window.setAlwaysOnTop`. Disabling minimal mode also clears always-on-top. In minimal mode, the sidebar is hidden.
- **Dropdown/overlay state**: all context menus and overlays (agent dropdown, workspace menu, skill menu, image viewer, file viewer, delete/archive/rename/duplicate dialogs, etc.) are managed via atoms and rendered at `AppLayoutContent` level. No prop-drilling for overlay state.
...
| Add a layout state flag | `LayoutProvider.tsx` — add to `LayoutState`, `LayoutContextValue`, and `value` object | Consumers use `useLayout()`. Panel geometry state (like width) belongs in dedicated atoms, not `LayoutProvider`. |
| Add a navigation item to the sidebar | `NavigationSection.tsx` | Use `NavItem` component |
...
## Related
- Depends on: [userData providers](../userData/), `agentChatSessionCacheManager` (`src/renderer/lib/chat/`), `LeftNavSizeAtom` / `LeftNavCollapsedAtom` (`src/renderer/states/left-nav.atom.ts`), `ResizableDivider` / `NavItem` UI primitives
