...
const RuntimeSettingsHeaderView: React.FC<RuntimeSettingsHeaderViewProps> = ({
  ...
  const { t } = useI18n()
  const installedStatus = t('settings.runtime.installed')
  const notInstalledStatus = t('settings.runtime.notInstalled')

  return (
    <div className="unified-header">
      <div className="header-title">
        <Terminal size={24} />
        <span className="header-name">{t('settings.navigation.runtime')}</span>
        <div className="mcp-status-badges">
          <Badge
            variant="normal"
            className="text-xs"
          >
            {t('settings.runtime.mode', { mode })}
          </Badge>
          <Badge
            variant={bunInstalled ? "normal" : "secondary"}
            className="text-xs"
          >
            {t('settings.runtime.bunStatus', { status: bunInstalled ? installedStatus : notInstalledStatus })}
          </Badge>
          <Badge
            variant={uvInstalled ? "normal" : "secondary"}
            className="text-xs"
          >
            {t('settings.runtime.uvStatus', { status: uvInstalled ? installedStatus : notInstalledStatus })}
          </Badge>
        </div>
      </div>
      <div className="header-actions">
        {onRefresh && (
          <button
            ...
            title={t('settings.runtime.refreshStatus')}
          >
