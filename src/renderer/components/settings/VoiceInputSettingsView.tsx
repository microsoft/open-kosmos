...
const VoiceInputSettingsView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubError = whisperApi.onDownloadError?.((data: { model: string; error: string }) => {
      setDownloadProgress(null)
      setError(tRef.current('settings.voiceInput.downloadFailed', { error: data.error }))
    })
  ...
  const handleSettingsChange = useCallback(async (newSettings: VoiceInputSettings) => {
    ...
    try {
      ...
      setError(t('settings.voiceInput.saveFailed', { error: err instanceof Error ? err.message : String(err) }))
    }
  }, [t])
  ...
  const handleToggleVoiceInput = useCallback(async (enabled: boolean) => {
    ...
    if (!enabled) {
      ...
      try {
        ...
        setError(t('settings.voiceInput.disableFailed', { error: String(err) }))
      }
      return
    }
    ...
    if (addonAlreadyDone && modelAlreadySelected) {
      try {
        ...
        setError(t('settings.voiceInput.enableFailed', { error: String(err) }))
      }
      return
    }
    ...
    try {
      ...
      if (!(err instanceof Error && err.name === 'AbortError')) {
        setEnablingError(t('settings.voiceInput.setupFailed', { error: err instanceof Error ? err.message : String(err) }))
      }
    } finally {
      setIsEnabling(false)
  ...
  const handleDownloadModel = useCallback(async (size: WhisperModelSize) => {
    try {
      ...
      setError(t('settings.voiceInput.startDownloadFailed', { error: err instanceof Error ? err.message : String(err) }))
    }
  }, [t])
  ...
  const handleDeleteModel = useCallback(async (size: WhisperModelSize) => {
    try {
      ...
      if (response?.success) {
        ...
        setError(t('settings.voiceInput.deleteModelFailed', { error: response?.error || t('common.unknownError') }))
      }
    } catch (err) {
      setError(t('settings.voiceInput.deleteModelFailed', { error: err instanceof Error ? err.message : String(err) }))
    }
  }, [settings.whisperModel, t])
