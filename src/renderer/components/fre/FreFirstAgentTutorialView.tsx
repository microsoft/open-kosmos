...
const logger = createLogger('[FreFirstAgentTutorialView]');
 ...
 * Shows welcome dialog after setup completion
 *
 * This view presents:
...
const FreFirstAgentTutorialView: React.FC<FreFirstAgentTutorialViewProps> = ({
    ...
    const loadVideoUrl = async () => {
      try {
        ...
        if (userDataPath) {
          ...
          logger.debug('[FRE][Tutorial] Local video URL:', fileUrl);
          setVideoUrl(fileUrl);
        } else {
          logger.error('[FRE][Tutorial] Failed to get userData path');
          setVideoError(true);
        }
      } catch (error) {
        logger.error('[FRE][Tutorial] Error loading video URL:', error);
        setVideoError(true);
      }
    };
  ...
  const handleCreateAgent = () => {
    logger.debug('[FRE][Tutorial] Create Agent button clicked');
  ...
  const handleExploreOnOwn = async () => {
    logger.debug('[FRE][Tutorial] Explore on own button clicked');
    ...
    try {
      ...
      if (userAlias && window.electronAPI?.profile?.updateFreDone) {
        ...
        logger.debug('[FRE][Tutorial] freDone updated to true');
      }
    } catch (error) {
      logger.error('[FRE][Tutorial] Error updating freDone:', error);
    }
          ...
          >
            ...
            {videoUrl && !videoError ? (
              <video
                ...
                playsInline
                onError={(e) => {
                  logger.error('[FRE][Tutorial] Video load error:', e);
                  setVideoError(true);
                }}
                style={{
                  width: '100%',
