...
export const VoiceInputButton: React.FC<VoiceInputButtonProps> = ({
      ...
      switch (update.type) {
        case 'interim':
          ...
          if (update.text && update.text !== lastTranscriptRef.current) {
            ...
            onTranscriptRef.current(update.text, false);
          }
          break;
        ...
        case 'final':
          ...
          if (update.text) {
            lastTranscriptRef.current = update.text;
            onTranscriptRef.current(update.text, true);
          }
          break;

        case 'error':
          logger.error('[VoiceInput] Streaming error:', update.error);
          setErrorMessage(update.error || tRef.current('chat.input.voiceTranscriptionError'));
          setStatus('error');
          setTimeout(() => {
            setStatus('idle');
  ...
  const handleClick = useCallback(async () => {
    ...
    if (isRecording) {
      ...
      try {
        ...
        setErrorMessage(err instanceof Error ? err.message : t('chat.input.voiceFailedToStop'));
        setStatus('error');
        setTimeout(() => {
          setStatus('idle');
          setErrorMessage(null);
      ...
      try {
        ...
        if (!response?.success || !response.data?.sessionId) {
          throw new Error(response?.error || t('chat.input.voiceFailedToStartStreaming'));
        }
        ...
        setErrorMessage(err instanceof Error ? err.message : t('chat.input.voiceFailedToStartRecording'));
        setStatus('error');
  ...
  const getTitle = (): string => {
    if (!isModelDownloaded) {
      return t('chat.input.voiceDownloadModel');
    }
    switch (status) {
      case 'listening':
        return t('chat.input.voiceStopListening');
      case 'processing':
        return t('chat.input.voiceProcessing');
      case 'error':
        return errorMessage || t('chat.input.voiceRecognitionError');
      default:
        return t('chat.input.voiceStart');
    }
  };
  ...
  return (
    <div className={`voice-input-container ${className}`}>
      ...
      >
        ...
        {!isModelDownloaded && (
          <div className="voice-input-setup-badge" title={t('chat.input.voiceSetupRequired')}>
            !
          </div>
        )}
      ...
      {/* Tooltip */}
      {/* v8 ignore next 5 -- showTooltip has no setter; this legacy placeholder is unreachable until hover support is wired. */}
      {showTooltip && (
